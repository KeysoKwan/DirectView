...
public class FAR : MonoBehaviour
{
    ...
    public void UpdateWindowPos(IntPtr ProjectionWindow)
    {
        ...
        if (temp_MonitorCount < 0)
            return;//fmARUpdatePhysicalMonitor报错
        ...
        for (int k = 0; k < temp_MonitorCount; k++)
        {
            ...
            if (result >= 0)
            {
                if (!gcinfo.isGCmonitor)
                {
                    ...
                    SetWindowPos(ProjectionWindow, -1, gcinfo.RCleft, gcinfo.RCtop, gcinfo.RCright - gcinfo.RCleft, gcinfo.RCbottom - gcinfo.RCtop, SWP_SHOWWINDOW);
                }
            }
            else
                return;//fmARGetMonitorInfoByIndex报错
        }
    }
